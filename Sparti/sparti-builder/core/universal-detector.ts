...
export class UniversalElementDetector {
  ...
  static extractElementData(element: HTMLElement): ElementData {
    ...
    try {
      switch (elementType) {
        case 'image':
          // Support both <img src="..."> and CSS background-image: url(...)
          const bg = computedStyles.backgroundImage;
          const bgMatch = bg && bg !== 'none'
            ? bg.match(/url\((?:"|')?([^"')]+)(?:"|')?\)/)
            : null;
          data.src = element.getAttribute('src') || bgMatch?.[1] || '';
